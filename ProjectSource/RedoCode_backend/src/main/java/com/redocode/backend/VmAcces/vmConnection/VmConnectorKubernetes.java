...
public class VmConnectorKubernetes extends VmConnector {
  ...
  public VmConnectorKubernetes() {
    try {
      ...
      if (kubernetesHost == null) kubernetesClient = new KubernetesClientBuilder().build();
      else {
      ...
      try {
        // TODO: 31/01/2024 repplace try catch with more reliable way of checking whether or
        // namespace already exist
        Namespace codeRunnerNameSpace =
            new NamespaceBuilder()
                .withNewMetadata()
                .withName(codeRunnersNamespace)
                .endMetadata()
                .build();
  ...
  Pod getPodFromID(String id) {
    logger.info(
        Arrays.toString(
            kubernetesClient.pods().inNamespace(codeRunnersNamespace).list().getItems().toArray()));
  ...
  public void destroyVm(String id) {
    ...
    this.kubernetesClient
        ...
        .waitUntilCondition(
            (deployment) -> {
              DeploymentList DepLisy =
                  kubernetesClient.apps().deployments().inNamespace(codeRunnersNamespace).list();

              List<Deployment> deployments = DepLisy.getItems();
              //                            logger.info("test:
              // "+deployments.get(0).getMetadata().getName());
              return deployments.stream()
                  .noneMatch(d -> Objects.equals(d.getMetadata().getName(), id));
            },
            30,
  ...
  public ConsoleOutput executeCommandInVm(String id, long timeout, String... command) {
    ...
    try {
      ...
      return new ConsoleOutput(
          exitCode, outputStream.toString().trim(), outputError.toString().trim());
    } catch (Exception ex) {
      ex.printStackTrace();
