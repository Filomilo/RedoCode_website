...
public class ExerciseTestFactory {
  ...
  private Object getSingleValue() {
    if (inputType.isInteger()) return generateInt();
    if (inputType.isFloat()) return generateFloat();
    if (inputType.isString()) return generateString();
    return null;
  }

  private Object[] getArrayValues() {
    int size =
        rnd.nextInt(this.xArrayRange.getMin().intValue(), this.xArrayRange.getMax().intValue());
  ...
  private Object[][] getDoubleArrayValues() {
    int sizeX =
        rnd.nextInt(this.xArrayRange.getMin().intValue(), this.xArrayRange.getMax().intValue());
    int sizeY =
        rnd.nextInt(this.yArrayRange.getMin().intValue(), this.yArrayRange.getMax().intValue());
    ...
    switch (inputType) {
      ...
      case ARRAY_OF_INTEGERS -> {
        Integer[] integerArray =
            Arrays.stream(getArrayValues()).map(x -> (Integer) x).toArray(Integer[]::new);

        return new ArrayOfIntegers(integerArray);
      }
      case ARRAY_STRINGS -> {
        String[] integerArray =
            Arrays.stream(getArrayValues()).map(x -> (String) x).toArray(String[]::new);

        return new ArrayOfStrings(integerArray);
      }
      case ARRAY_OF_FLOATS -> {
        Float[] floatArray =
            Arrays.stream(getArrayValues()).map(x -> (Float) x).toArray(Float[]::new);
