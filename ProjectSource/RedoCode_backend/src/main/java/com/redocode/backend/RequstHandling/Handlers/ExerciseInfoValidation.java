...
public class ExerciseInfoValidation extends MessageRequestHandler {
  ...
  RequestBase handle(RequestBase request) throws RequestHadndlingException {
    this.nodeUpdate(
        request, "Checking exercise information", ChainNodeInfo.CHAIN_NODE_STATUS.RUNNING);
    ...
    if (!TitlePattern.matcher(exerciseCretionRequest.getTitle()).matches()) {
      throw new RequestHadndlingException(
          "Exercise title: " + exerciseCretionRequest.getTitle() + " is inocrrect");
    }
    ...
    if (exerciseCretionRequest.getAmountOfAutoTests() <= 0
        || exerciseCretionRequest.getAmountOfAutoTests() > 10) {
      throw new RequestHadndlingException("inccorrect Amount of auto test");
    }

    if (exerciseCretionRequest.getInputType().getId() > 2
        && exerciseCretionRequest.getXArrayRange() != null) {
    ...
    if (exerciseCretionRequest.getInputType().getId() > 5
        && exerciseCretionRequest.getYArrayRange() != null) {
    ...
    switch (exerciseCretionRequest.getInputType().getId() % 3) {
      case 2:
        if (!validlateArrayRange(
            exerciseCretionRequest.getLengthRange(), Integer.MIN_VALUE, Integer.MAX_VALUE)) {
          throw new RequestHadndlingException(
              "inccorrect int values range: " + exerciseCretionRequest.getLengthRange());
        }
        break;
      case 0:
        if (!validlateArrayRange(
            exerciseCretionRequest.getLengthRange(), Integer.MIN_VALUE, Integer.MAX_VALUE)) {
          throw new RequestHadndlingException(
              "inccorrect float values range: " + exerciseCretionRequest.getLengthRange());
        }
        break;
      case 1:
        if (!validlateArrayRange(exerciseCretionRequest.getLengthRange(), 0, 30)) {
          throw new RequestHadndlingException(
              "inccorrect string values range: " + exerciseCretionRequest.getLengthRange());
        }
        break;
    }

    if (exerciseCretionRequest.getTimeForExecution() < 10
        || exerciseCretionRequest.getTimeForExecution() > 5000) {
