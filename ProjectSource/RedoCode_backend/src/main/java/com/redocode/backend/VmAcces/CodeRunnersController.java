...
public class CodeRunnersController {
  ...
  @Autowired private MessageSender messeageSender;
  ...
  public void deregisterUser(User user) {
    ...
    Optional<CodeRunnerRequest> requestMessage =
        requestMessageSet.stream().filter((req) -> req.getUser().equals(user)).findFirst();
  ...
  public CodeRunner getUserCodeRunner(User user) {
    ...
    log.info(
        "hashmap hashes: "
            + Arrays.toString(
                this.usersCodeRunenrs.keySet().stream().map(a -> a.hashCode()).toArray()));
  ...
  public void updateCodeRunnerState(User user) {
    ...
    if (messeageSender != null) {
      ...
      CoderunnerStateMessage coderunnerStateMessage =
          CoderunnerStateMessage.builder()
              .state(state)
              .codeRunnerType(
                  userCodeRunner == null ? CODE_RUNNER_TYPE.UNIDENTIFIED : userCodeRunner.getType())
              .build();
      log.info("user: " + user + " requested status: " + coderunnerStateMessage);
      messeageSender.sendMessage(
          user, CodeRunnersConnectionController.codeRunnerStateEndPoint, coderunnerStateMessage);
    }
  }
  ...
  public List<ProgramResult> runCode(User user, ExerciseIdToRunMessage exerciseIdToRunMessage) {
    ...
    if (codeRunner == null) throw new RuntimeException("user doesnt have code runner");
  ...
  public List<ProgramResult> runProgramFromMessage(
      CodeRunner codeRunner, ExerciseIdToRunMessage exerciseIdToRunMessage) {
  ...
  private List<ProgramResult> runRawProgramFromMessage(
      CodeRunner codeRunner, ExerciseIdToRunMessage exerciseIdToRunMessage) {
    Program pr;
  ...
  private List<ProgramResult> runExerciseSoultionFromMessage(
      CodeRunner codeRunner, ExerciseIdToRunMessage exerciseIdToRunMessage) {
    List<ProgramResult> results = new ArrayList<>();
    log.info("Ruunnign code to run on exercise of id: " + exerciseIdToRunMessage.getExercise_id());
    Excersize exercise =
        exerciseRepository
            .findById(Long.parseLong(exerciseIdToRunMessage.getExercise_id()))
            .orElse(null);
    ...
    try {
      for (ExerciseTests test : tests) {
        ...
        if (result.getVariables() == null
            || result.getVariables().getValue() != output.getValue()) {
  ...
  public void sendResults(User user, List<ProgramResult> results) {
    ...
    this.messeageSender.sendMessage(
        user, CodeRunnersConnectionController.codeRunnerResultEndPoint, results);
  }

  public void removeAllCodeRunners() {
    usersCodeRunenrs.forEach(
        (User user, CodeRunner cosdeRuner) -> {
          cosdeRuner.destroy();
        });
  }
