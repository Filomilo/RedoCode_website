...
public class VmConnectorDocker extends VmConnector {
  ...
  VmConnectorDocker() throws VmControllerException {
    ...
    DockerClientConfig dockerConfiguration =
  ...
  public String createVm(String Image, int ram) throws ContainerException {
    ...
    log.info(
        "Successfully created docker conaitner with image: "
            + Image
            + " with id: "
  ...
  public VmStatus getVmStatus(String id) {
    try {
      ...
      if (status.contains("Up")) return VmStatus.RUNNING_MACHINE;
      if (status.contains("Exited")) return VmStatus.MACHINE_STOPPED;
  ...
  List<String> contianerListToIdList(List<Container> contiaenrList) {
    return contiaenrList.stream().map(Container::getId).toList();
  }
  ...
  public ConsoleOutput executeCommandInVm(String id, long timeout, String... command)
    ...
    if (!outputStream.toString().isEmpty()) {
      ...
      } else output = outputStream.toString();
    }
