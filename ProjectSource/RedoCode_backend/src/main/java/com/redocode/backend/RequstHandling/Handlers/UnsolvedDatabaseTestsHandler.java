...
public class UnsolvedDatabaseTestsHandler extends  MessageRequestHandler {
    ...
    RequestBase handle(RequestBase request) throws RequestHadndlingException {
        ...
        List<ExerciseTests> testsToGenerateExpectedOutput= testsToRunRequest.getAutotestsToRun().stream().filter(x->x.getExpectedOutput()==
                null).toList();
        ...
        PorgramReusltsSendRequest resultsfromCodeTest= (PorgramReusltsSendRequest) codeTestHandler.handle(codeTestRequest);

        log.info("resultsfromCodeTest: "+resultsfromCodeTest);
            final int[] i = {0};

        testsToRunRequest.getAutotestsToRun().stream().filter(x->x.getExpectedOutput()==null).forEach(
                x->{
                    try{
                        log.info("UnsolvedDatabaseTestsHandler results: "+resultsfromCodeTest.getProgramResults().get(i[0]));
                    x.setExpectedOutput(RedoCodeObjectMapper.VarAsString(resultsfromCodeTest.getProgramResults().get(i[0]).getVariables()));
                    x.setInput(RedoCodeObjectMapper.VarAsString(resultsfromCodeTest.getProgramResults().get(i[0]).getVariablesInput()));
i[0]++;} catch (JsonProcessingException e) {
