...
public class CodeRunnerControllerOld {
  ...
  //    public String test(Principal principal, SimpMessageHeaderAccessor sha,
  // @Header("simpSessionId")String sessionIdProvider, String test) throws Exception{
  //
  //        log.info("atrtributes : "+
  // Arrays.toString(sha.getSessionAttributes().keySet().toArray()));
  ...
  ////                    template.convertAndSendToUser(sessionId,"/codeRunners/messages", payload,
  ...
  //                    //                    this.sendMesseage("Messege from : "+sessionid+":
  // "+test+" send conut: "+i);
