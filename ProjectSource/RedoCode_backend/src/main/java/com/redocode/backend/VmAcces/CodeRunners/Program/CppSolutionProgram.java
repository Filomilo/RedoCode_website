...
public class CppSolutionProgram extends SolutionProgram {
    ...
    String returnfunc =
        switch (var.getType()) {
          case ARRAY_STRINGS, ARRAY_OF_FLOATS, ARRAY_OF_INTEGERS -> {
            ...
            String tmp = "return " + getVarName(var.getType()) + " {";
          ...
          case DOUBLE_ARRAY_OF_FLOATS, DOUBLE_ARRAY_OF_INTEGERS, DOUBLE_ARRAY_OF_STRINGS -> {
            Object[][] arr = (Object[][]) var.getValue();
            String singleVar =
                getVarName(var.getType()).substring(0, getVarName(var.getType()).length() - 2);
            Variables.VARIABLES_TYPES singleArrType =
                switch (var.getType()) {
  ...
  public String getInputGeneratorCode() {
    String inputCodeGenerationExpected =
        "#include <iostream>\n"
            + "#include <vector>\n"
            + getVarName(getInput().getType())
            + " "
            + this.getInputGeneratorFunctionName()
            + "()\n"
            + "{\n"
  ...
  public String getOutputGeneratorCode() {
    String code =
        "#include <fstream>\n"
            ...
            + "#include <vector>\n"
            + "void "
            + this.getOutputGeneratorFunctionName()
            + "("
            + getVarName(this.getOutputType())
            + " a)\n"
            + "{\n"
            + "std::ofstream myfile;\n"
            + "myfile.open (\""
