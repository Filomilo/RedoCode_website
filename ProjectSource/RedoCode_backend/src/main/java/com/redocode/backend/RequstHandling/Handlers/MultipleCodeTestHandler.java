...
public class MultipleCodeTestHandler extends MessageRequestHandler {
  ...
  RequestBase handle(RequestBase request) throws RequestHadndlingException {
    ...
    codeTester.setMehthodForChain(
        (User user, ExecutionResponseBase executionResponseBase) -> {
          ExecutionResponseStatusUpdate statusUpdate =
              (ExecutionResponseStatusUpdate) executionResponseBase;
    ...
    List<Map.Entry<CODE_RUNNER_TYPE, String>> lists =
        new ArrayList<>(exerciseCreationRequest.getSolutionCodes().entrySet());
    lists.sort(Comparator.comparing(Map.Entry::getValue));
    for (Map.Entry<CODE_RUNNER_TYPE, String> entry : lists) {
      try {
        log.info(
            "ruuning test on code runner "
                + entry.getKey()
                + " WIth  code : \n "
                + entry.getValue()
                + "\n");
        ...
        if (!res) throw new RequestHadndlingException("test faliure");
