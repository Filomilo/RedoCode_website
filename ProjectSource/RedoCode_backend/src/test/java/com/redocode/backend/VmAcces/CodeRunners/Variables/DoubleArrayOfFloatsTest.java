...
class DoubleArrayOfFloatsTest {
  ...
  void deserialzationTest() {
    ...
    DoubleArrayOfFloats res =
        (DoubleArrayOfFloats)
            RedoCodeObjectMapper.parseVaraibles(
                bytes, Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_FLOATS);
  ...
  void StringifyTest() {
    ...
    DoubleArrayOfFloats res =
        (DoubleArrayOfFloats)
            RedoCodeObjectMapper.parseVaraibles(
                string, Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_FLOATS);
    assertEquals(doubleArrayOfFloats, res);
  }
  ...
  void desreialzeBadCase() {
    assertDoesNotThrow(
        () -> {
          String jsonString =
              "[[0.9199068,0.01825869],[0.03128314,0.94099855],[0.87412083,0.5238099],[0.109776914,0.92646646],[0.9567671,0.06990063],[0.39623928,0.6877511]]";

          ObjectMapper mapper = new ObjectMapper();
          DoubleArrayOfFloats result =
              (DoubleArrayOfFloats)
                  RedoCodeObjectMapper.parseVaraibles(
                      jsonString, Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_FLOATS);
