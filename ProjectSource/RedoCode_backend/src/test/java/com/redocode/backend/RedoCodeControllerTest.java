...
class RedoCodeControllerTest {
  ...
  void addConnectedUser() {
    ...
    assertTrue(
        redoCodeController.connectedUsers.values().stream().anyMatch(user -> (user.equals(user1))));
    assertTrue(
        redoCodeController.connectedUsers.values().stream().anyMatch(user -> (user.equals(user2))));
  }
  ...
  void removeConnectedUser() {
    ...
    log.info(
        "checking doesContainUser1: "
            + Arrays.toString(redoCodeController.connectedUsers.keySet().toArray()));
    Boolean doesContainUser1 =
        redoCodeController.connectedUsers.values().stream().anyMatch(user -> (user.equals(user1)));
    log.info(
        "checking doesContainUser2: "
            + Arrays.toString(redoCodeController.connectedUsers.keySet().toArray()));

    Boolean doesContainUser2 =
        redoCodeController.connectedUsers.values().stream().anyMatch(user -> (user.equals(user2)));
    redoCodeController.removeConnectedUser("2");
    Boolean doesContainUser2AfterRemoval =
        redoCodeController.connectedUsers.keySet().stream().anyMatch(user -> (user.equals(user2)));
