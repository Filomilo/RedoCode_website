...
//        exerciseTestsRepository.saveAll(tests);
        ...
        solutionProgramsRepository.save(
                SolutionPrograms.builder()
                        ...
                        .language(programmingLanguageRepository.findByName(RedoCodeObjectMapper.CodeRunnerToDataBaseLanguageName(CPP_RUNNER)))
                        .excersize(excersize)
                        .build()
        );
        solutionProgramsRepository.save(
                SolutionPrograms.builder()
                        ...
                        .language(programmingLanguageRepository.findByName(RedoCodeObjectMapper.CodeRunnerToDataBaseLanguageName(CODE_RUNNER_TYPE.JS_RUNNER)))
                        .excersize(excersize)
                        .build()
        );
    ...
    @ParameterizedTest
    @EnumSource(value = CODE_RUNNER_TYPE.class, mode = EnumSource.Mode.EXCLUDE, names = {"UNIDENTIFIED"})
    public void RunExerciseIdCodeCorrect(CODE_RUNNER_TYPE type){
        User user = new User();
        long IdOfeExercise=exerciseRepository.findAll().get(0).getId();

        String solutionCode=solutionProgramsRepository.findByLanguageIdAndExerciseId(
                programmingLanguageRepository.findByName(RedoCodeObjectMapper.CodeRunnerToDataBaseLanguageName(type)).getId()
                ,IdOfeExercise
        ).getCode();
        ...
        SingleDatabaseExerciseTestRequest singleDatabaseExerciseTestRequest = SingleDatabaseExerciseTestRequest
                .builder()
    ...
    public void testRetruNTheSameCorect(){
        ...
        String solutionCode=solutionProgramsRepository.findByLanguageIdAndExerciseId(
                programmingLanguageRepository.findByName(RedoCodeObjectMapper.CodeRunnerToDataBaseLanguageName(CPP_RUNNER)).getId()
                ,IdOfeExercise
        ).getCode();
