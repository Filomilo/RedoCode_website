...
class ExerciseTestsRepositoryTest {
  ...
  void saveExerciseTest() throws JsonProcessingException {
    ...
    ExerciseTests test =
        ExerciseTests.builder().input(inputString).expectedOutput(outputString).build();
    ...
    SingleInteger inputFromTest =
        (SingleInteger)
            RedoCodeObjectMapper.parseVaraibles(
                lastAdded.getInput(), Variables.VARIABLES_TYPES.SINGLE_INTEGER);
    ArrayOfIntegers ouptutFromTest =
        (ArrayOfIntegers)
            RedoCodeObjectMapper.parseVaraibles(
                lastAdded.getExpectedOutput(), Variables.VARIABLES_TYPES.ARRAY_OF_INTEGERS);
