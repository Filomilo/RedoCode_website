...
public class ValuesProvider {
  ...
  private static Integer[][] generateRadnomIntDoubleArray(int height, int width) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) arr[i][j] = rand.nextInt();
    }
    return arr;
  }
  ...
  private static Float[][] generateRadnomFloatDoubleArray(int height, int width) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) arr[i][j] = rand.nextFloat();
    }
    return arr;
  }
  ...
  private static String[][] generateRadnomStringDoubleArray(int height, int width) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) arr[i][j] = genRandString();
    }
    return arr;
  }
  ...
  public static Stream<Arguments> arrayIntProvider() {
    ...
    Stream<Arguments> args =
        argeumnets.stream()
            .map(
                (arg) -> {
                  return Arguments.of((Object) arg);
                });
    return args;
  }
  ...
  public static Stream<String> singleStringProvider() {
    ...
    argeumnets.add(
        "int main()\n"
            ...
            + "21:14:00.464 [main] INFO com.redocode.backend.VmAcces.CodeRunners.CodeRunnerTest --"
            + " Code to run: \n"
            + "\n"
            + "\n"
    ...
    for (int i = 0; i < tests.length; i++) {
      ...
      argeumnets.add(
          ((SingleString) tests[i].getParsedInput(Variables.VARIABLES_TYPES.SINGLE_STRING))
              .getValue());
    }
  ...
  public static Stream<Arguments> doubleArrayStringProvider() {
    ...
    int height = rand.nextInt(1, 20);
    //        argeumnets.add(new
    // String[][]{{"AAAAAAAAAAAA","BBBBBBBBBBBBBB"},{"CCCCCCCCCCC","DDDDDDDDDDDDDD"}});
    argeumnets.add(new String[][] {{"AAAAAAAAAAAA", "\n"}, {"CCCCCCCCCCC", "DDDDDDDDDDDDDD"}});
    //         argeumnets.add(new
    // String[][]{{"AAAAAAAAAAAA","\t"},{"CCCCCCCCCCC","DDDDDDDDDDDDDD"}});
