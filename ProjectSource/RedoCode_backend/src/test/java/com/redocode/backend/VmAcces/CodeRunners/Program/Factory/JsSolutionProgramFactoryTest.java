...
public class JsSolutionProgramFactoryTest {
  ...
  void getInputGeneratorCodeSingleInt(Integer var) {
    ...
    String inputCodeGenerationExpected =
        "function "
            + program.getInputGeneratorFunctionName()
            + "()\n"
            + "{\n"
            + "return "
            + var
    ...
    assertEquals(
        ...
        program.getInputGeneratorCode(),
        "inputArgs funciton doenst match one prepread in template");
  }
  ...
  void getInputGeneratorCodeArrayInt(Integer[] var) {
    ...
    for (int i = 0; i < var.length; i++) {
      ...
      if (i < var.length - 1) inputCodeGenerationExpected += ", ";
    }
    inputCodeGenerationExpected += "];\n" + "}";
    log.info("code: \n" + inputCodeGenerationExpected);
    assertEquals(
  ...
  void getInputGeneratorCodeDoubleArrayInt(Integer[][] var) {
    ...
    for (int i = 0; i < var.length; i++) {
      ...
      for (int j = 0; j < var[0].length; j++) {
        ...
        if (j < var[0].length - 1) inputCodeGenerationExpected += ", ";
      }
      inputCodeGenerationExpected += "]";
      if (i < var.length - 1) inputCodeGenerationExpected += ", ";
    }
  ...
  void getInputGeneratorCodeSingleString(String var) {
    ...
    String inputCodeGenerationExpected =
        "function "
            ...
            + "{\n"
            + "return \""
            + StringFormatter.removeWhiteCharacterss(var)
            + "\";\n"
            + "}";
    log.info("code: \n" + inputCodeGenerationExpected);
    assertEquals(
  ...
  void getOutputGeneratorCodeSingleString() {
    ...
    String ouputGenerationCode =
        " const "
            + program.getOutputGeneratorFunctionName()
            + "_fs = require('fs');\n"
            + "function "
            + program.getOutputGeneratorFunctionName()
            + "(a)\n"
            ...
            + "        {\n"
            + "            a=a.map(element => {return element.toString().replace(/\\\\/g,"
            + " \"\\\\\\\\\").replace(/\\n"
            + "/g, \"\\\\n"
            + "\").replace(/\\t/g, \"\\\\t\")});\n"
            + "            str=a.join(\"\\t\");\n"
            ...
            + "        else{\n"
            + "            a=a.map(element => {\n"
            + "                element=element.map(x=>{return x.toString().replace(/\\\\/g,"
            + " \"\\\\\\\\\").replace(/\\n"
            + "/g, \"\\\\n"
            + "\").replace(/\\t/g, \"\\\\t\")})\n"
  ...
  void getActivationFunctionMultiArrayAsArumgument() {
    SolutionProgram program =
        ProgramFactory.createSolutionProgram()
            .setSolutionCodeRunner(CODE_RUNNER_TYPE.JS_RUNNER)
            .setInputVaraiable(
                new DoubleArrayOfIntegers(new Integer[][] {{1, 8, 5, 4}, {1, 8, 5, 4}}))
            .setTimeout(500)
            .setOutputBase(Variables.VARIABLES_TYPES.SINGLE_INTEGER)
            .setSolutionCode("")
            .build();
  ...
  void FibonachiCodeTest() {
    ...
    SolutionProgram solutionProgram =
        ProgramFactory.createSolutionProgram()
            .setSolutionCodeRunner(
                CODE_RUNNER_TYPE
                    .JS_RUNNER) // TODO: add class mapping data base string to Code runner enum
            .setInputVaraiable(new SingleInteger(1))
            .setOutputBase(Variables.VARIABLES_TYPES.SINGLE_INTEGER)
            .setTimeout(500)
    ...
    String correctCode =
        "function solution(val) {\n"
            ...
            + "\n"
            + "function "
            + solutionProgram.getInputGeneratorFunctionName()
            + "()\n"
            + "{\n"
            + "return 1;\n"
            + "}\n"
            + "\n"
            + " const "
            + solutionProgram.getOutputGeneratorFunctionName()
            + "_fs = require('fs');\n"
            + "function "
            + solutionProgram.getOutputGeneratorFunctionName()
            + "(a)\n"
            ...
            + solutionProgram.getOutputGeneratorFunctionName()
            + "_fs.writeFileSync(\""
            + solutionProgram.getOutputFileName()
            + "\",str);\n"
            + "}\n"
            + "\n"
            + solutionProgram.getOutputGeneratorFunctionName()
