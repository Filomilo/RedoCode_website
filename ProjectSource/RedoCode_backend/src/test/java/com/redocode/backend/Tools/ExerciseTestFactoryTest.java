...
class ExerciseTestFactoryTest {
  ...
  void generateInts() {
    ...
    ExerciseTests[] test =
        new ExerciseTestFactory()
            .setAmount(aamount)
            .setInputType(Variables.VARIABLES_TYPES.SINGLE_INTEGER)
            .setLengthRange(new Range(min, max))
            .build();
    assertEquals(aamount, test.length);
    for (int i = 0; i < aamount; i++) {
      SingleInteger singleInteger =
          (SingleInteger) test[i].getParsedInput(Variables.VARIABLES_TYPES.SINGLE_INTEGER);
  ...
  void generatefloats() {
    ...
    assertEquals(aamount, test.length);
    for (int i = 0; i < aamount; i++) {
      SingleFloat singleInteger =
          (SingleFloat) test[i].getParsedInput(Variables.VARIABLES_TYPES.SINGLE_FLOAT);
      log.info(singleInteger.toString());
      assertTrue(singleInteger.getValue() <= max && singleInteger.getValue() >= min);
    }
  }
  ...
  void generateStringLengthTest() {
    ...
    for (int i = 0; i < aamount; i++) {
      SingleString singleString =
          (SingleString) test[i].getParsedInput(Variables.VARIABLES_TYPES.SINGLE_STRING);
      log.info(singleString.toString());
      assertTrue(
          singleString.getValue().length() <= max && singleString.getValue().length() >= min);
    }
  }
  ...
  void generateStringCapitalLetters() {
    ...
    for (int i = 0; i < aamount; i++) {
      ...
      for (int j = 0; j < singleString.getValue().length(); j++) {
        ...
        assertTrue((int) character <= 'Z' && character >= 'A');
      }
    }
  }
  ...
  void generateStringNumbers() {
    ...
    for (int i = 0; i < aamount; i++) {
      ...
      for (int j = 0; j < singleString.getValue().length(); j++) {
        Character character = (Character) singleString.getValue().charAt(j);
        assertTrue((int) character <= '9' && character >= '0');
      }
    }
  }
  ...
  void generateIntsArray() {
    ...
    for (int i = 0; i < aamount; i++) {
      ArrayOfIntegers arrayOfIntegers =
          (ArrayOfIntegers) test[i].getParsedInput(Variables.VARIABLES_TYPES.ARRAY_OF_INTEGERS);
      log.info(arrayOfIntegers.toString());
      assertTrue(
          arrayOfIntegers.getValue().length <= maxX && arrayOfIntegers.getValue().length >= min);
    }
  }
  ...
  void generateFloatArray() {
    ...
    for (int i = 0; i < aamount; i++) {
      ArrayOfFloats arrayOfFloats =
          (ArrayOfFloats) test[i].getParsedInput(Variables.VARIABLES_TYPES.ARRAY_OF_FLOATS);
  ...
  void generateStringArray() {
    ...
    for (int i = 0; i < aamount; i++) {
      ArrayOfStrings arrayOfStrings =
          (ArrayOfStrings) test[i].getParsedInput(Variables.VARIABLES_TYPES.ARRAY_STRINGS);
      log.info(arrayOfStrings.toString());
      assertTrue(
          arrayOfStrings.getValue().length <= maxX && arrayOfStrings.getValue().length >= min);
    }
  }
  ...
  void generateIntsDoubleArray() {
    ...
    assertEquals(aamount, test.length);
    for (int i = 0; i < aamount; i++) {
      DoubleArrayOfIntegers doubleArrayOfIntegers =
          (DoubleArrayOfIntegers)
              test[i].getParsedInput(Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_INTEGERS);
      log.info(doubleArrayOfIntegers.toString());
      assertTrue(
          doubleArrayOfIntegers.getValue().length <= maxX
  ...
  void generateFloatDoubleArray() {
    ...
    assertEquals(aamount, test.length);
    for (int i = 0; i < aamount; i++) {
      DoubleArrayOfFloats doubleArrayOfFloats =
          (DoubleArrayOfFloats)
              test[i].getParsedInput(Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_FLOATS);
      log.info(doubleArrayOfFloats.toString());
      assertTrue(
          doubleArrayOfFloats.getValue().length <= maxX
    ...
    for (int i = 0; i < aamount; i++) {
      DoubleArrayOfStrings doubleArrayOfStrings =
          (DoubleArrayOfStrings)
              test[i].getParsedInput(Variables.VARIABLES_TYPES.DOUBLE_ARRAY_OF_STRINGS);
      log.info(doubleArrayOfStrings.toString());
      assertTrue(
          doubleArrayOfStrings.getValue().length <= maxX
