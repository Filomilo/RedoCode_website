  ...
  void startStopContainer() {
    assertDoesNotThrow(
        () -> {
          ...
          // TODO: 02/02/2024 create namespaces for stastus shared between docker and kubernetes
          // management
          // assertEquals("Up",containerStatusAfterRunning.split(" ",2)[0],"Container has a wrong
  ...
  void executeCommandInVm() {
    ...
    assertDoesNotThrow(
        () -> {
          ...
          String res =
              vmConnectorDocker.executeCommandInVm(id, 100, "echo", checkpharase).getOutput();
          ...
          assertEquals(
              checkpharase,
              res,
              "Command execution result does not much provided echo phraseL " + checkpharase);
          assertEquals(
              amtOfConatinersBefore,
              amtOfConatinersAfterestrcution,
              "New Vm failed to bew removed after creation");
  ...
  void executeProgramInputInVMInVm() {
    ...
    assertDoesNotThrow(
        () -> {
          ...
          String res =
              vmConnectorDocker
                  .executeCommandInVmWithInput(id, "/inputOutput", argumentsInput)
                  .getOutput();
          ...
          assertEquals(
              argumentsInput, res, "Testing program output did not match provided arguments");
          assertEquals(
              amtOfConatinersBefore,
              amtOfConatinersAfterestrcution,
              "New Vm failed to bew removed after creation");
