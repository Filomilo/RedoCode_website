...
class GetExerciseDataFromDataBaseTest {
  ...
  void handle() {
    ...
    CodeRunnerRequest codeRunnerRequest =
        CodeRunnerRequest.builder().user(user).codeRunnerType(CODE_RUNNER_TYPE.CPP_RUNNER).build();
    ...
    SpecifiedSingleDatabaseExerciseTestRequest specifiedSingleDatabaseExerciseTestRequest =
        (SpecifiedSingleDatabaseExerciseTestRequest)
            getExerciseDataFromDataBase.handle(singleDatabaseExerciseTestRequest);

    assertEquals(
        excersize.getInputType(), specifiedSingleDatabaseExerciseTestRequest.getInputType());
    assertEquals(
        excersize.getOutputType(), specifiedSingleDatabaseExerciseTestRequest.getOutputType());
    assertEquals(
        excersize.getMaxExecutionTimeMS(),
        specifiedSingleDatabaseExerciseTestRequest.getTimeForExecution());
    assertEquals(excersize.getRam_mb(), specifiedSingleDatabaseExerciseTestRequest.getRam());

    for (int i = 0; i < exerciseTests.size(); i++) {
      assertEquals(
          exerciseTests.get(i).getInput(),
          specifiedSingleDatabaseExerciseTestRequest.getTestsToRun().get(i).getInput());
      assertEquals(
          exerciseTests.get(i).getExpectedOutput(),
          specifiedSingleDatabaseExerciseTestRequest.getTestsToRun().get(i).getExpectedOutput());
    }
