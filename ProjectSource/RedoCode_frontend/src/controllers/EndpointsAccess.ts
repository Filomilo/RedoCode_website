...
namespace EndpointAccess {
  ...
  export namespace authorized {
    ...
    export async function postAccountPic(image: string): Promise<string> {
      try {
        const data = {
          image: image,
        }
    ...
    export async function getUserDetails(): Promise<UserDetailsMessage> {
      try {
        console.log('attempting /secure/user/details ')
        const response = await axios.get('/secure/user/details')
        if (response.status !== 200 && response.status !== 201)
          throw "couldn't get detail data"
    ...
    export async function postChangeDescription(desc: string) {
      try {
        ...
        console.log(
          'response /secure/user/description: ' + JSON.stringify(response)
        )
        if (response.status !== 200 && response.status !== 201)
    ...
    export async function getUserInfo(): Promise<AccountInfo> {
      try {
        ...
        console.log(
          'getUserInfo: response.data ' + JSON.stringify(response.data)
        )
        return response.data
      } catch ({ response }: any) {
        throw response.data
