<template>
  ...
  <div class="CodeResultContainer">
    ...
    >
      <div class="executionTimeContainer" v-if="executionTimeString.length > 0">
        <p>
          {{ executionTimeString }}
        </p>
      </div>
...
<script setup lang="ts">
  ...
  const formattedError = computed<string>(() => {
    ...
    const manualTests = props.ManualTests as ConsoleOutput | undefined
    return manualTests
      ? StringParser.parseStringToCodeResult(manualTests.errorOutput)
      : ''
  })

  const executionTimeString = computed<string>(() => {
    return props.ExecutionTime === undefined ||
      props.ExecutionTime === null ||
      props.ExecutionTime < 0
      ? ''
