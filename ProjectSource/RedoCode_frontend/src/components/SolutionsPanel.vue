<template>
  <div class="SolutionsContainer">
    <div class="LeftSideBar-solutions">
      ...
      :solutionList="props.solutionList" :maxExecutionTime="props.maxExectuionTime"
      />
...
const loadSolutionCode=async (solutionId: number)=>{
  code.value = '';
  try {
    const codeIn = await EndpointsAcces.authorized.getSolutionsCodesData(solutionId,ActiveUserStore.getToken());
...
const props = defineProps<{
  ...
  const lang=computed(()=>{
    const type: CodeRunnerType|undefined=   props.solutionList.find(x=>x.solutionId===selectedSolutionId.value)?.codeRunner;
    if(type===undefined)
    return ""
