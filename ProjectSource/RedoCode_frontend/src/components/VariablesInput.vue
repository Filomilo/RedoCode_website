...
<script lang="ts" setup>
  ...
  const storedValue = computed(() => {
    return props.isInput
      ...
      : CodeRunnerStore.manualTestBuffer[props.manualTestInputIndex]
          .expectedOutput
  })
  ...
  const validateData = (jsonString: string, type: VarType): boolean => {
    ...
    try {
      value =
        props.Type == 'SINGLE_STRING' ? jsonString : JSON.parse(jsonString)
    ...
    try {
      ...
      if (validationofData.value === '') {
        if (props.isInput) {
          ...
          CodeRunnerStore.manualTestBuffer[
            props.manualTestInputIndex
          ].expectedOutput = newVal
        }
      }
    } catch (error) {
      console.error(error)
