describe('hello world js run ', () => {
  it('passes', () => {
    Cypress.on('uncaught:exception', (err, runnable) => {
      return false
    })
    const codeEditorSequance = '#coderunner-editor-panel textarea'

    const helloWorldProgram = 'console.log("Hello World!")'

    cy.visit('/')
    cy.get('#switch-playground').click()
    cy.url().should('eq', Cypress.config().baseUrl + '/PlayGround')
    cy.get('#coderunner-dropdown').click()
    cy.get('#coderunner-dropdown_1').click()
    // eslint-disable-next-line cypress/no-unnecessary-waiting

    cy.get('.p-button-label').click()
    // cy.get("PlayGroundBase div.heightLimit div.p-splitter.p-component.p-splitter-horizontal div.p-splitter-panel div.CodeResultContainer div.EngineStatusContianer div.EngineStatusPanel div.EngineStatusStatus").contains('ACTIVE')
    cy.get('#coderunner-loading-dialog').should('not.exist')
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(20)
    cy.get(codeEditorSequance).focus()
    cy.get(codeEditorSequance).type('{selectAll}', { force: true })
    for (let i = 0; i < helloWorldProgram.length; i++) {
      cy.get(codeEditorSequance).focus()
      cy.get(codeEditorSequance).type(helloWorldProgram[i], { force: true })
    }

    cy.get('#coderunner-run-button').click()

    cy.get(
      'html body div#app html div#MainPageContainer div.BackGroundContainer main.PlayGroundBase div.heightLimit div.p-splitter.p-component.p-splitter-horizontal div.p-splitter-panel div.CodeResultContainer div.ConsoleResultConsoleCOntainer div.ConsoleResultConsoleCOntainerText'
    ).contains('Hello World!')
  })
})
