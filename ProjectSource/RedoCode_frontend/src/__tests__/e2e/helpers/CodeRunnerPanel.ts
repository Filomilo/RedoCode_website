  ...
  export namespace Tests {
    ...
    export function shouldAllTesCorrect(amountOfTest: number) {
      ...
      for (let index = 0; index < amountOfTest; index++) {
        ...
        cy.get(`#tab-result-achieved-container-${index}`)
          .should('exist')
          .invoke('text')
          .then(achivedText => {
            cy.get(`#tab-result-expected-container-${index}`)
              .should('exist')
              .invoke('text')
              .then(expectedText => {
                ...
                cy.log(
                  `expeted:: [ ${expectedString} ] equals to [ ${achivedStrinf} ] `
                )
    ...
    export function compareExpectedANdResult(
      fun: (epxected: any, result: any) => boolean
    ) {
          ...
          .then(achivedText => {
            cy.get(`#tab-result-expected-container-${index}`)
              ...
              .then(expectedText => {
                ...
                cy.log(
                  `expected: [ ${expectedString} ] --- achived: [ ${achivedStrinf} ]`
                )
                // cy.pause();
              })
          })
