...
namespace AccountPanel {
  ...
  export function changeDescription(
    ...
    if (shouldBeCorrect)
      ...
      ToastHelper.shouldHaveErrorToast(
        "Couldn't change description: Description too long, max 3000 characters"
      )
  ...
  export function shouldHaveBasePicture() {
    cy.get('#profile-image').should(
      'have.attr',
      'src',
      '/src/assets/Images/ProfilePictureBase.png'
    )
  }
  ...
  export function changeProfilePicture(imageLocation: string) {
    ...
    cy.get('#choose-image-button > span > input[type=file]').selectFile(
      Cypress.config('projectRoot') + '/src/__tests__/e2e/assets/imgtemp.png',
      { force: true }
    )
