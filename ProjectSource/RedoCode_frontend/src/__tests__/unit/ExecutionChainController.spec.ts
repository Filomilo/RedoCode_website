...
describe('Exercsie creation controller tests', () => {
    ...
    const executionChainController: ExecutionChainController= new ExecutionChainController(stompApiSubsciptionContorlle); 
  ...
  it('loading chain Scheeme', () => {
    ...
    for (let index = 0; index < ExecutionChainTemplate.levels.length; index++) {
        expect(executionChainController.executionChain[index]).toBe(ExecutionChainTemplate.levels[index])
  ...
  it('load updates in correct order',async ()=>{
    for (let index = 0; index < CorrectUpdates.length; index++) {
       ...
       await executionChainController.updateStatus(CorrectUpdates[index])
        console.log("Finsehs")
        expect (executionChainController.executionChain[nodeIndex].processingMessage).toBe(CorrectUpdates[index].message);
        expect  (executionChainController.executionChain[nodeIndex].status).toBe( CorrectUpdates[index].lvlStatus);
        expect(executionChainController.shouldBeVisible).toBeTruthy();
  ...
  for (let index = 1; index < ExecutionChainTemplate.levels.length; index++) {
    expect(executionChainController.executionChain[index].status).toBe(ExecutionChainTemplate.levels[index].status)
