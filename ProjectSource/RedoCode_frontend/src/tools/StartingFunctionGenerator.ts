...
const getCppImport = (
  inputVaraible: VarType,
  outputVariable: VarType
): string => {
  let imports = ''

  if (
    isTypeArray(inputVaraible) ||
    isTypeDoubleArray(inputVaraible) ||
    isTypeArray(outputVariable) ||
    isTypeDoubleArray(outputVariable)
  ) {
...
const generateCppFunction = (
  inputVaraible: VarType,
  outputVariable: VarType
): string => {
  let func = ''
  func +=
    getCppImport(inputVaraible, outputVariable) +
    getVarCpVarType(outputVariable) +
    ' solution(' +
    getVarCpVarType(inputVaraible) +
    ' x){\n //your solution \n}'

  return func
}

export default function generateStartingFunction(
  codeRunnerType: CodeRunnerType,
  inputVariable: VarType,
  outputVariable: VarType
) {
  console.log(
    'starting function generator: ' + inputVariable + ', ' + outputVariable
  )
  switch (codeRunnerType) {
    case CodeRunnerType.CPP_RUNNER:
      return generateCppFunction(inputVariable, outputVariable)
    case CodeRunnerType.JS_RUNNER:
      return generateJsFunction()
    default:
